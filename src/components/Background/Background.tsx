...
import { saveFile, loadFile } from './handleStorage.ts'
...
const Background: Component = () => {
  ...
  const updateBackground: JSX.EventHandler<
    HTMLInputElement,
    InputEvent
  > = event => {
    ...
    const updateBackground = async (): Promise<void> => {
      if (files != null) {
        setbackgroundURL(
          URL.createObjectURL(await saveFile(files[0], 'background'))
        )
      }
    }
  ...
  return (
    <div>
      ...
      <label for={inputId} class={styles.inputLabel}>
        <div style={{ display: 'block', width: '40px', color: 'currentColor' }}>
          <svg
      ...
      <Show when={backgroundURL()}>
        <img class={styles.background} src={backgroundURL()} loading="eager" />
      </Show>
    </div>
  )
}
